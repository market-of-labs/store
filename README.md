...
**新增单是两拍的** —— 因为模板里只剩"申请人**知道**的东西"：

| 拍 | 何时 | 做什么 | 申请人看到什么 |
|---|---|---|---|
| **1 · 登记**（秒级，纯本地） | 一提交就发车 | 只做本地校验（仓库形状、正则是否编译得过、勾选项是否在固定词表内）→ 打「待收录」标签 | 「已收到」+ 一句"身份在对账时读出来" |
| **2 · 对账**（每日） | 下一轮全量对账 | 去上游读出**包名**（APK 的 `package`）、**显示名**（APK 的 `label`）、**作者**（仓库 owner）→ 落盘 `sources/` → 回评核对 → **关单** | 一张表格列出读出来的身份，请核对 |
...
**issue 本身就是队列**：标签即状态（「待收录」→「待补充」/关单），而**标签跳变**才是回评的触发点。
所以每天扫一遍不会把回评刷成墙纸 —— 状态没变的单一条评论都不加。想改内容就**编辑正文**，
不必重开单（编辑后的正文走的是**同一套**解析规则，见 `internal/job/pending.go`）。
...
| 3 | 本仓库与 `forge` 各配一份 **同名** secret `GH_PAT`，两份是**同一把** fine-grained PAT，仓库范围必须含 `store`(Contents RW + Issues RW) / `forge`(Contents RW) / **`forge-core`(Contents R)** |
